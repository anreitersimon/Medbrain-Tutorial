...
\section{Project Setup}\label{step-1---project-setup}
...
CocoaPods\cite{cocoapods}}\label{step-2---managing-dependencies-with-cocoapods}
...
FHIR-Resources}\label{step-3---overview-of-used-fhir-resources}
...
\item
  MedicationOrder \ref{res:medication-order}
\item
  DosageInstructions \ref{res:dosage-instructions}
\item
  Timing \ref{res:timing}
\item
  MedicationAdministration \ref{res:medication-administration}
\end{itemize}

These will be explained more in depth in the corresponding
section below.

\subsection{Resources}\label{step-3-resources}
...
\subparagraph{This resources is referenced by:}\label{referenced-by}
...
\subsubsection{Medication}\label{medication}
...
\paragraph{This resource is referenced by:}\label{referenced-by}
...
\subsubsection{MedicationOrder}\label{medicationorder}
...
\subsection{MedicationAdministration}\label{medicationadministration}
...
Structure}\label{step-4---building-the-application-structure}

\subsection{Prerequisites}\label{prerequisites}

Finished \href{STEP2.md}{Step 2} of this Tutorial.

If you want to start here you just run \texttt{git\ checkout\ step4}.
...
\section{Implementing SignIn}\label{step-5---implementing-signin}

\subsection{Prerequisites}\label{prerequisites}

Finished \href{STEP4.md}{Step 4} of this Tutorial.

If you want to start here you just run \texttt{git\ checkout\ step5}.
...
Interface}\label{step-5.1---implementing-signin---patientsigninviewcontroller-interface}
...
\subsubsection{Building the Interface}\label{building-the-interface}
...
\subsubsection{Adding outlets to the class implementation}\label{add-the-outlets-to-the-class}
...
\subsubsection{Adding the UI-Elements in the storyboard}\label{in-the-storyboard-add-the-elements.}
\begin{quote}
...
\subsection{PatientSignInViewController functionality}\label{step-5.2---implementing-signin---patientsigninviewcontroller-functionality}

\subparagraph{\texorpdfstring{Import \texttt{SMART}
framework}{Import SMART framework}}\label{import-smart-framework}

add \texttt{import\ SMART} statement to the top of the
\texttt{PatientSignInViewController}

\subsubsection{Adding a completionHandler}\label{add-a-completionhandler}
...
\section{Implementing PatientMedicationsViewController}\label{step-6---implementing-patientmedicationsviewcontroller}

\subsection{Prerequisites}\label{prerequisites}

If you want to start here you just run \texttt {git\ checkout\ step6}.
...
\subsubsection{Defining outlets in the implementation class}\label{define-the-outlets-in-the-implementation.}
...
Finished \href{STEP6.md}{Step 6} of this Tutorial.

If you want to start here you just run \texttt{git\ checkout\ step7}.
...
\subsubsection{Defining the outlets and actions}\label{defining-the-outlets-and-actions}
...
\subsubsection{Implementing the Chart}\label{implementing-the-chart}
...
\paragraph{Generating identifiers}\label{generating-identifiers}
...
\paragraph{Creating the Charts Data-Model}\label{creating-the-charts-data-model}
...
\paragraph{Implementing the updateChartData() function}\label{implementing-the-updatechartdata-function}
...
\paragraph{Implementing the ORKGraphChartViewDataSource protocol methods}\label{implementing-the-orkgraphchartviewdatasource-protocol-methods}
...
You: - added a new dependency (\texttt{ResearchKit}) - implemented a
list showing all \texttt{MedicationAdministrations} - loaded all
\texttt{MedicationAdministrations} for a specific prescription from a
remote server - implemented a graph visualizing the timeline of
administrations

\section{Implementing the CreateMedicationAdministrationViewController}\label{step-8---implementing-createmedicationadministrationviewcontroller}

\subsection{Prerequisites}\label{prerequisites}

Finished \href{STEP7.md}{Step 7} of this Tutorial.

If you want to start here you just run \texttt{git\ checkout\ step8}.
...
This Step is split up into two parts: - Building the Interface. -
...
\paragraph{Setting up the
ViewController}\label{setting-up-the-viewcontroller}
